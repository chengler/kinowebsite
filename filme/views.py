...
def film_index(request):
    ...
    if nextevents:
        ...
        events = Event.objects.filter(event_online=True, termin__date=datum).order_by(
            "termin"
        )
    else:
...
def filmevent_programm_list(request):
    ...
    return render(
        request,
        "filme/filmevent_programm_list.html",
        {
            "events": nextevents,
            "sidebar_events": sidebar_events,
            "sidebar_count": sidebar_count,
...
def newsletters_opt_in(request, email, salz):
    ...
    return render(
        request, "filme/newsletter_opt_in.html", {"email": email, "status": status}
    )
...
def newsletter_unsubscribe(request, email, salz):
    ...
    return render(
        request, "filme/newsletter_unsubscribe.html", {"email": email, "status": status}
    )
...
def newsletter_render_today(request):
    ...
    return render(
        request,
        "filme/newsletter_render_today.html",
        {"events": events, "hinweis": hinweis},
    )
...
def sondernews_send(request, pk, email="@all"):
    ...
    if email == "@all":
        ...
        logger.info(
            "sondernews_send:  sonder_newsletter.gesendet  %s",
            sonder_newsletter.gesendet,
        )
    ...
    return render(
        request,
        "filme/newsletter_send.html",
        {
            "start": newsletter.gestartet,
            "ende": newsletter.beendet,
            "count": newsletter.anzahl,
            "timediff": "in der mail",
            "key": "via login",
            "typ": "Sondernewsletter",
        },
    )
...
def newsletter_send(request, typ, key):
    ...
    if typ == "heute":
        # überprüfe das Password
        if key == HEUTE_KEY:
            # lösche Karteileichen
            logger.debug("newsletter_send: checke Karteileichen")
            karteileichen = NewsletterAbonnent.objects.filter(
                beantragt__lte=datetime.datetime.now() - datetime.timedelta(days=2)
            ).exclude(opt_in=True)
            logger.info(
                "newsletter_send: entferne %s Karteileichen.", karteileichen.count()
            )
            ...
            if datetime.datetime.now().date() == datum:
                ...
                return render(
                    request,
                    "filme/newsletter_send.html",
                    {"key": "heute kein Film, nix gesendet"},
                )
        else:
        ...
        if key == HINWEIS_KEY:
            if Inhaltsseite.objects.get(typen="1").anzeigen == True:
                ...
                return render(
                    request,
                    "filme/newsletter_send.html",
                    {"key": "heute kein Hinweis, nix gesendet"},
                )
        else:
...
def event_publish(event):
    ...
    print(
        "event.event_film_confirmed ",
        event.event_film_confirmed,
        "event.film.film_komplett ",
        event.film.film_komplett,
    )
...
def get_programm_query():
    ...
    return Event.objects.filter(
        event_online=True, termin__gte=datetime.datetime.now()
    ).order_by("termin")
    ...
    for kategorie in Event.KATEGORIE_CHOICES:
        if (
            Event.objects.filter(
                event_online=True, termin__gte=mydate, kategorie=kategorie[0]
            ).count()
            > 0
        ):
            nextobject = Event.objects.filter(
                event_online=True, termin__gte=mydate, kategorie=kategorie[0]
            ).order_by("termin")[0]
...
def film_draft_bewertung(request, pk):
    ...
    return render(
        request, "filme/film_draft_bewertung.html", {"form": form, "film": film}
    )
...
def film_spielplanung_list(request, kat=0):
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            ...
            set_termin(
                events
            )  # mache datetime  und korrigiere event_online falls nötig
            # setze online
            formset.save()
        events_next = get_spielplanung_query(kategorieauswahl)
        ...
        formset = SpielplanFormSet(queryset=events_next)
    return render(
        request, "filme/film_spielplanung.html", {"formset": formset, "filme": filme}
    )
...
def get_spielplanung_query(kategorieauswahl):
    ...
    if kategorieauswahl != 0:
        return (
            Event.objects.filter(termin__gte=datetime.datetime.now())
            .filter(kategorie=kategorieauswahl)
            .order_by("termin")[:100]
        )
    else:  # alle kategorien
        return Event.objects.filter(termin__gte=datetime.datetime.now()).order_by(
            "termin"
        )[:100]
...
def set_termin(events):
    ...
    for idx, i in enumerate(events):
        ...
        events[idx].termin = datetime.datetime.combine(
            events[idx].termin, events[idx].zeit
        )
...
def film_spielplan_list(request):
    ...
    events = Event.objects.filter(termin__gte=datetime.datetime.now()).order_by(
        "termin"
    )
...
def filmevent_archiv(request):
    ...
    events = Event.objects.filter(termin__lte=datetime.datetime.now()).order_by(
        "-termin"
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if "film_komplett" in form.changed_data and film.film_komplett:
                ...
                for event in film.event_film.all().filter(
                    termin__gte=datetime.datetime.now()
                ):
                    event_publish(event)
            next = request.GET.get("next", "/")
...
def sondernewsletter_list(request):
    ...
    return render(
        request,
        "filme/newsletter_sondernewsletter_list.html",
...
def sondernewsletter_edit(request, pk):
    ...
    logger.debug(
        "sondernewsletter_edit:  request.user  %s newsletter.bild_thumb.url %s",
        request.user,
        newsletter.bild_thumb.url,
    )
    return render(
        request,
        "filme/newsletter_sondernewsletter_edit.html",
        {"user": request.user, "newsletter": newsletter, "form": form},
    )
...
def sondernewsletter_render(request, pk):
    ...
    return render(
        request,
        "filme/newsletter_render_sondernewsletter.html",
...
def film_detail(request, pk):
    ...
    return render(
        request,
        "filme/film_detail.html",
        {"film": film, "form": form, "sidebar_events": sidebar_events},
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if event.last_change > got:
                ...
                messages.error(
                    request,
                    "Mist, die Datenbank wurde in der Zwischenzeit geändert. Hier sind die aktuellen Daten. Bitte nochmal!",
                )
        ...
        form = EventDetailForm(instance=event)
    return render(
        request, "filme/filmevent_detail_edit.html", {"form": form, "event": event}
    )
...
def filmevent_dienste(request, pk):
    ...
    if request.method == "POST":
        ...
        form = EventDetailForm(instance=event)
    return render(
        request, "filme/filmevent_dienste.html", {"form": form, "event": event}
    )
...
def film_mufi(request):
    inhalt = Inhaltsseite.objects.filter(typen=6)
    inhalt = inhalt[0]
    return render(request, "filme/inhaltsseite.html", {"inhalt": inhalt})
...
def intern(request):
    ...
    for group in groups:
        ...
        try:  # Hole id und name der Rollendokus der auth_group
            rollendokus = list(
                Rollendoku.objects.filter(group_id=group.pk).values_list("id", "name")
            )
